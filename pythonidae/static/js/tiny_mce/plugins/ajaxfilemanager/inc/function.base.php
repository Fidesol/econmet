...
function myRealPath($path) {
 ...
 function getUrl($filename)
 {
		...
		error_log ("PROTOCOLO ".$pos);
		return $protocol . "://" .  backslashToSlash($_SERVER['HTTP_HOST']) . CONFIG_ABSOLUTE_PATH .$filename; 	
 }
	...
	function getFileUrl($value)
	{
		...
		error_log('SUPRAAA '.getBaseName($value));
		$value = backslashToSlash(getRealPath($value));
		...
		$protocol = (isset($_SERVER["HTTPS"]) &&  $_SERVER["HTTPS"] == 'on' ? 'https' : 'http');
		error_log ("PROTOCOLO ".$pos);
		return $protocol . "://" .  backslashToSlash($_SERVER['HTTP_HOST']) . CONFIG_ABSOLUTE_PATH .$value;
